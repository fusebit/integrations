...
router.get('/install', async (req, res, next) => {
  try {
    ...
    const createSessionResponse = await fetch(`${res.locals.config.integrationUrl}/session`, {
      body,
...
router.get('/callback', async (req, res, next) => {
  ...
  try {
    const sessionPersistResponse = await fetch(`${integrationUrl}/session/${sessionId}/commit`, {
      headers: getHeaders(res),
      method: 'POST',
    });
