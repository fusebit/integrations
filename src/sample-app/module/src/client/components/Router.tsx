...
const Routes = () => {
  ...
  const handleLogin = async (tenant: Tenant) => {
    const response = await fetch('{{APP_URL}}/api/tenants/login', {
  ...
  const AuthedRouteWithProps = (props: { path: string } & RouteProps) => (
    <AuthedRoute {...props} onLogin={handleLogin} tenantData={tenantData} />
  );

  const FrameWithProps = (props: RouteProps) => <Frame {...props} onLogout={handleLogout} tenantData={tenantData} />;
