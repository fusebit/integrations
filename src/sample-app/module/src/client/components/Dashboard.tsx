...
export default (props: {isInstalled: boolean, isActive: boolean}) => {
    ...
    const alert = () => {
        ...
        const installedMessage = props.isActive ? "A message is being sent to your slack account." : "Enable the installed Slack integration to receive messages";
        const message = props.isInstalled ? installedMessage :"Head to the Integration Marketplace to install the Slack Integration";

        setAlertProps({ severity, text: message});
    }

    const Body = () => hasLoaded ? (
