...
test('stackoverflow-provider test', async ({ page }) => {
  ...
  const fastExit = await authenticate(page);
  if (fastExit) {
    // Captcha and the like keeps this from completeing fully.  Nothing to be done about it in automation.
    return;
  }
