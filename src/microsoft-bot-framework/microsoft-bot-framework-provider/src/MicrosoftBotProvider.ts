...
export default class MicrosoftBotProvider extends Internal.ProviderActivator<FusebitBotFrameworkAdapter> {
  ...
  public async instantiate(ctx: Internal.Types.Context): Promise<FusebitBotFrameworkAdapter> {
    ...
    botFrameworkAdapter.processActivity = async (
      ...
      // The botbuilder SDK expects the body to contain the original event, and if we don't change it here
      // we would actually be passing a body that contains Fusebit properties, along with some credentials
      // generated by MS Bot Framework Connector. Apart from that, botbuilder also expects req.headers
